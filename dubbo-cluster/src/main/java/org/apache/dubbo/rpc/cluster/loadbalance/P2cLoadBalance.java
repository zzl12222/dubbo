...
public class P2cLoadBalance extends AbstractLoadBalance {
    ...
    public P2cLoadBalance(ApplicationModel scopeModel) {
    ...
    protected <T> Invoker<T> doSelect(List<Invoker<T>> invokers, URL url, Invocation invocation) {
        ...
        Invoker<T> invoker = chooseLowLoadInvoker(invokers.get(pos1), invokers.get(pos2), invocation);
        invocation.setAttachment(Constants.P2C_LOADBALANCE_ATTACHMENT_KEY, attachmentKey);
        long startTime = System.currentTimeMillis();
        invocation.getAttributes().put(Constants.P2C_LOADBALANCE_START_TIME, startTime);
        invocation.getAttributes().put(LOADBALANCE_KEY, LoadbalanceRules.P2C);
        p2cMetrics.addConsumerReq(getServiceKey(invoker, invocation));
        p2cMetrics.setPickTime(getServiceKey(invoker, invocation), startTime);
        return invoker;
    }
    ...
    private <T> Invoker<T> chooseLowLoadInvoker(Invoker<T> invoker1, Invoker<T> invoker2, Invocation invocation) {
        ...
        long load1 =
                Double.doubleToLongBits(p2cMetrics.getLoad(getServiceKey(invoker1, invocation), weight1, timeout1));
        long load2 =
                Double.doubleToLongBits(p2cMetrics.getLoad(getServiceKey(invoker2, invocation), weight2, timeout2));
